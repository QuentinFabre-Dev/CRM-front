...
export default function AssessmentPage({ params }: { params: { id: string } }) {
  ...
  const baselineLabel =
    assessment.templateName ?? BASELINES.find((b) => b.id === assessment.baseline)?.label ?? assessment.baseline;
  ...
  return (
    <div className="flex h-[calc(100vh-8rem)] flex-col space-y-4">
        ...
        <div className="mt-2 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="mt-0.5 text-[12.5px] text-muted-foreground">
              {assessment.templateName ? "Modèle " : "Baseline "}
              {baselineLabel}
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="flex flex-1 flex-col overflow-hidden">
        ...
        <TabsContent value="actifs" className="flex-1 overflow-y-auto">
          <AssetsTab assessmentId={assessmentId} />
        </TabsContent>
